...
FINAL="$BUILD/ACL_DEMO_FINAL_V6_NO_1M32S_PAUSE.mov"

# Remove the residual low-energy pause between the annotation introduction and
# “We annotate the player,” preserving a short natural breath.
ffmpeg -hide_banner -loglevel error -y -i "$SOURCE" -filter_complex \
  "[0:v]trim=start=0:end=92.08,setpts=PTS-STARTPTS[v0];
   [0:a]atrim=start=0:end=92.08,asetpts=PTS-STARTPTS[a0];
   [0:v]trim=start=92.36:end=230.40,setpts=PTS-STARTPTS[v1];
   [0:a]atrim=start=92.36:end=230.40,asetpts=PTS-STARTPTS[a1];
